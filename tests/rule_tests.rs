...
use graph_generation_lang::parser::{Pattern, NodeDeclaration, EdgeDeclaration};
...
mod simple_rule_tests {
    ...
    fn test_node_deletion() {
        ...
        graph.add_edge(Edge::new("e1".to_string(), "connected1".to_string(), "connected2".to_string()));
...
mod attribute_based_tests {
    ...
    fn test_attribute_matching() {
        ...
        old_attrs.insert("status".to_string(), MetadataValue::String("active".to_string()));
        ...
        let mut graph = Graph::new();
        graph.add_node(Node::new("n1".to_string())
            .with_metadata("status".to_string(), MetadataValue::String("active".to_string())));
        graph.add_node(Node::new("n2".to_string())
            .with_metadata("status".to_string(), MetadataValue::String("inactive".to_string())));
        graph.add_node(Node::new("n3".to_string())
            .with_metadata("status".to_string(), MetadataValue::String("active".to_string())));
        ...
        assert!(n2.metadata.get("timestamp").is_none());
    ...
    fn test_boolean_attribute_matching() {
        ...
        update_attrs.insert("reason".to_string(), MetadataValue::String("deactivated_by_rule".to_string()));
        ...
        // Check results
        let n1 = graph.get_node("n1").unwrap();
        assert_eq!(n1.metadata.get("enabled"), Some(&MetadataValue::Boolean(false)));
        assert_eq!(n1.metadata.get("reason"), Some(&MetadataValue::String("deactivated_by_rule".to_string())));

        let n2 = graph.get_node("n2").unwrap();
        assert_eq!(n2.metadata.get("enabled"), Some(&MetadataValue::Boolean(false))); // Unchanged
        assert!(n2.metadata.get("reason").is_none());
    }
}
...
mod edge_pattern_tests {
    ...
    fn test_edge_transformation() {
        ...
        graph.add_node(Node::new("n2".to_string()));
        graph.add_node(Node::new("n3".to_string()));
        graph.add_edge(Edge::new("e1".to_string(), "n1".to_string(), "n2".to_string()));
        graph.add_edge(Edge::new("e2".to_string(), "n2".to_string(), "n3".to_string()));
...
mod complex_pattern_tests {
    ...
    fn test_star_to_cycle() {
        ...
        graph.add_edge(Edge::new("e1".to_string(), "center".to_string(), "leaf1".to_string()));
        graph.add_edge(Edge::new("e2".to_string(), "center".to_string(), "leaf2".to_string()));
        graph.add_edge(Edge::new("e3".to_string(), "center".to_string(), "leaf3".to_string()));
...
mod rule_application_tests {
    ...
    fn test_rule_termination() {
        ...
        assert_eq!(graph.node_count(), 3);
        for (_, node) in &graph.nodes {
            assert_eq!(node.metadata.get("processed"), Some(&MetadataValue::Boolean(true)));
        }
    }
}
...
mod performance_tests {
    ...
    fn test_large_graph_rule_application() {
        ...
        assert_eq!(graph.node_count(), 100);
        for (_, node) in &graph.nodes {
            assert_eq!(node.metadata.get("marked"), Some(&MetadataValue::Boolean(true)));
        }
    }
}
