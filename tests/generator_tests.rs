...
mod complete_graph_tests {
    ...
    fn test_complete_graph_basic() {
        ...
        for i in 0..5 {
            ...
            assert!(graph.get_node(&node_id).is_some(), "Node {} should exist", node_id);
        }
    }
...
mod generator_registry_tests {
    ...
    fn test_all_generators_callable() {
        let generators = [
            "complete", "path", "cycle", "grid",
            "star", "tree", "barabasi_albert"
        ];
        ...
        for gen_name in generators {
            ...
            assert!(result.is_ok(), "Generator {} failed with minimal params: {:?}",
                   gen_name, result.err());
        }
    }
}
...
mod parameter_validation_tests {
    ...
    fn test_parameter_types() {
        ...
        let mut params = HashMap::new();
        params.insert("nodes".to_string(), MetadataValue::String("not_a_number".to_string()));
    ...
    fn test_edge_properties() {
        ...
        for (_, edge) in &graph.edges {
            assert!(graph.get_node(&edge.source).is_some(),
                   "Edge source {} should exist", edge.source);
            assert!(graph.get_node(&edge.target).is_some(),
                   "Edge target {} should exist", edge.target);
