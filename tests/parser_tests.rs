...
mod lexical_tests {
    ...
    fn test_identifiers() {
        ...
        assert!(result.is_ok(), "Failed to parse valid identifiers: {:?}", result.err());

        let statements = result.unwrap();
        assert_eq!(statements.len(), 4);

        // Verify all are node declarations with correct IDs
        for (i, expected_id) in ["simple_id", "CamelCase", "with_123_numbers", "_underscore_start"].iter().enumerate() {
            match &statements[i] {
    ...
    fn test_strings() {
        ...
        let result = parse_ggl(input);
        assert!(result.is_ok(), "Failed to parse strings: {:?}", result.err());

        let statements = result.unwrap();
        assert_eq!(statements.len(), 4);

        let expected_labels = ["simple string", "string with spaces", "string_with_underscores", ""];
    ...
    fn test_numbers() {
        ...
        let result = parse_ggl(input);
        assert!(result.is_ok(), "Failed to parse numbers: {:?}", result.err());
    ...
    fn test_booleans() {
        ...
        let result = parse_ggl(input);
        assert!(result.is_ok(), "Failed to parse booleans: {:?}", result.err());
    ...
    fn test_rule_with_attributes() {
        ...
        match &statements[0] {
            GGLStatement::RuleDefStmt(rule) => {
                ...
                assert_eq!(rule.lhs.nodes[0].attributes.get("status"),
                          Some(&MetadataValue::String("old".to_string())));
                assert_eq!(rule.rhs.nodes[0].attributes.get("status"),
                          Some(&MetadataValue::String("new".to_string())));
                assert_eq!(rule.rhs.nodes[0].attributes.get("updated"),
                          Some(&MetadataValue::Boolean(true)));
            }
            _ => panic!("Expected RuleDefStmt"),
        }
