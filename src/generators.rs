...
fn get_param_int(params: &HashMap<String, MetadataValue>, key: &str) -> Result<usize, String> {
    match params.get(key) {
        Some(MetadataValue::Number(n)) => Ok(*n as usize),
...
pub fn generate_barabasi_albert(params: &HashMap<String, MetadataValue>) -> Result<Graph, String> {
    ...
    // Add initial complete graph with m nodes
    for i in 0..m {
        let node_id = format!("{}{}", prefix, i);
        graph.add_node(Node::new(node_id));
    }
    ...
    for i in m..n {
        ...
        for j in 0..i {
            ...
            let degree = graph.edges.values()
                ...
                .count();
            degrees.push((node_id, degree));
        }
        
        // Select m distinct nodes with probability proportional to degree
        let mut selected = Vec::new();
        while selected.len() < m {
            let total_degree: usize = degrees.iter().map(|(_, d)| d).sum();
            let mut r = rng.gen_range(0..total_degree);
            
            for (node_id, degree) in degrees.iter() {
                if r < *degree && !selected.contains(node_id) {
        ...
        for target in selected {
            let edge_id = format!("e{}_{}", i, target.trim_start_matches(&prefix));
