...
pub fn parse_ggl(input: &str) -> Result<Vec<GGLStatement>, String> {
    let pairs = GGLParser::parse(Rule::program, input)
    ...
    match pair.as_rule() {
        Rule::statement => {
            ...
            match inner.as_rule() {
                ...
                Rule::generate_stmt => Ok(Some(GGLStatement::GenerateStmt(parse_generate_stmt(inner)?))),
...
fn parse_rule_def(pair: pest::iterators::Pair<Rule>) -> Result<RuleDefinition, String> {
    let mut name = String::new();
    let mut lhs = Pattern { nodes: Vec::new(), edges: Vec::new() };
    let mut rhs = Pattern { nodes: Vec::new(), edges: Vec::new() };
...
fn parse_apply_rule(pair: pest::iterators::Pair<Rule>) -> Result<ApplyRuleStatement, String> {
    ...
    for inner_pair in pair.into_inner() {
        match inner_pair.as_rule() {
            ...
            Rule::number => {
                iterations = inner_pair.as_str().parse::<usize>()
...
fn parse_attributes(pair: pest::iterators::Pair<Rule>) -> Result<HashMap<String, MetadataValue>, String> {
    let mut attributes = HashMap::new();
